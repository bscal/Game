	...
	public boolean checkCollision(Rectangle rect, int layer, int xZoom, int yZoom) {
		...
		for(int x = topLeftX; x < bottomRightX; x++) {
			for(int y = topLeftY; y < bottomRightY; y++ ) {
				...
				if(tile != null) {
	 				int type = tileSet.getCollisionType(tile.id);	//Collision Type
	 				//Collision Types goes by 0 = All sides, 1 = Top, 2 = left, 3 = Bottom, 4 = Right, 5 = LowerHalf
	 				//6 = All sides but flat (Projectiles/Flying can pass over)
					
					if(type == 0) {
						Rectangle tileRect = new Rectangle(tile.x * tileWidth, tile.y * tileHeight, tileWidth, tileHeight);
						...
						Rectangle tileRect = new Rectangle(tile.x * tileWidth + tileWidth - 16, tile.y * tileHeight, 16, tileHeight);
						if(tileRect.intersects(rect)) {
							return true;
						}
					}
					else if(type == 5) {
						Rectangle tileRect = new Rectangle(tile.x * tileWidth, tile.y * tileHeight + tileHeight - 16, tileWidth, 42);
		...
		else 
		{
			...
			for(int x = 0; x < grids.length; x++)
				for(int y = 0; y < grids[0].length; y++)
					if(grids[x][y] != null) 
					{
						newBlocks[x + (startX - newMinX)/gridWidth][y + (startY - newMinY)/gridHeight] = grids[x][y];
					}
